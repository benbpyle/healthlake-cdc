...
export class PipelinePrepStack extends Stack {
    ...
    constructor(scope: Construct, id: string, props: PipelinePrepStackProps) {
        ...
        new CfnEventBusPolicy(this, `${props?.options.stackNamePrefix}-${props?.options.stackName}-EventsPolicy`, {
            statementId: `${props?.options.stackNamePrefix}-${props?.options.stackName}-CodeCommit`,
            eventBusName: 'default',
            statement: {
                Effect: 'Allow',
                Principal: { AWS: `arn:aws:iam::${props.options.codeCommitAccount}:root` },
...
export class CodeCommitStack extends Stack {
    ...
    constructor(scope: Construct, id: string, props: CodeCommitStackProps) {
        ...
        new CfnRule(this, `${props?.options.stackNamePrefix}-${props?.options.stackName}-UpdateToPipeline`, {
            description: 'Send CodeCommit events to Pipeline Account',
            eventBusName: 'default',
            eventPattern: {
                'detail-type': ['CodeCommit Repository State Change'],
                source: ['aws.codecommit'],
                resources: [`arn:aws:codecommit:${this.region}:${this.account}:${props.options.reposName}`],
            },
            state: 'ENABLED',
            targets: [{
